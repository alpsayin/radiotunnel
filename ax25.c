...
uint8_t ax25_check_destination(uint8_t* my_dst, uint8_t* packet_dst_out, uint8_t* packet_in)
{
    ...
    result=strncmp(my_dst, packet_in+AX25_DESTINATION_OFFSET, AX25_DESTINATION_LENGTH);
    if(result)
    {
        result=strncmp(ax25_broadcast_address, packet_in+AX25_DESTINATION_OFFSET, AX25_DESTINATION_LENGTH);
    }
